import numpy as np
from config import BLOSUM, AMINO_ACIDS, DICT_PATH, PATH
import pickle
import pdb

class PriorDist:
    def __init__(self, device):
        dist_dict = pickle.load(open(DICT_PATH, 'rb'))
    ...
    def get_pos(self, sequence, length, peptide):
        ...
        peptide_dist = np.array([dist[i, a-1] for i, a in enumerate(peptide) if a > 0])
        pos_dist = max_dist - peptide_dist
