...
class MySubprocVecEnv(VecEnv):
    ...
    def step(self, actions: np.ndarray, alleles: list) -> VecEnvStepReturn:
        """
        Step the environments with the given action
        :param actions: the action
        :return: observation, reward, done, information
        """
        self.step_async(actions, alleles)
        return self.step_wait()       

    def step_async(self, actions: np.ndarray, alleles: list) -> None:
        for remote, action, allele in zip(self.remotes, actions, alleles):
            remote.send(("step", (action, allele)))
        self.waiting = True
...
if __name__ == "__main__":
    ...
    m_env_kwargs = {"action_space":action_space, "observation_space":observation_space, \
                ...
                "max_step":args.max_step, "allow_imm_rew": 0} 
    ...
    obs = m_env.reset([True] * len(batch_alleles), batch_alleles, [None] * len(batch_alleles))
    rollout = 0
    ...
    while rollout < len(rollout_alleles):
        ...
        new_obs, rewards, dones, infos = m_env.step(actions, batch_alleles)
        ...
        #pdb.set_trace()
        obs = m_env.reset(dones, batch_alleles, new_obs) 
        ck_time = time.time()
